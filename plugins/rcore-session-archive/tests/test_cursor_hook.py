...
class CursorSetupTests(unittest.TestCase):
    ...
    def test_checked_in_hooks_match_installer_and_ignore_private_files(self):
        ...
        result = subprocess.run(["git", "check-ignore", ".cursor/session-archive.json", ".cursor/hooks.json", ".cursor/rcore-hooks/cursor_hook.py"], cwd=REPOSITORY, text=True, capture_output=True)
