...
function archive(payload) {
  return new Promise((resolve, reject) => {
    const child = spawn("python3", [join(root, ".opencode/rcore-hooks/opencode_hook.py"), "--project", root], {
...
const CourseSessionArchive = async ({ client, directory }) => {
  ...
  async function report() {
    try {
      await client.app.log({ body: { service: "rcore-session-archive", level: "warn",
...
export default {
  ...
  setup: async (context) => {
    ...
    const report = () => console.warn("rcore-session-archive: OpenCode 归档未完成；请检查 Python 3、项目配置及归档目录权限。");
