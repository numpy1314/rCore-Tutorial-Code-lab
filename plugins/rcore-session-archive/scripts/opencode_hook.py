...
def install_hooks(root):
    ...
    paths = [config, config / "plugins", config / "rcore-hooks"]
    sources = Path(__file__).resolve().parent
    files = [(sources / name, config / "rcore-hooks" / name)
             for name in ("archive_session.py", "archive_storage.py", "opencode_hook.py")]
    files.append((sources / "opencode_plugin.js", config / "plugins/rcore-session-archive.js"))
    ...
    private_directory(config / "rcore-hooks")
...
def main():
    ...
    try:
        ...
        print(f"rcore-session-archive: OpenCode 归档未完成（{type(error).__name__}）；请检查项目配置及归档目录权限。", file=sys.stderr)
        return 1
    return 0
