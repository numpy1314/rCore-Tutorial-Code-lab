...
本项目提供 `rcore-session-archive`，支持 Codex、Claude Code、Cursor、VS Code
...
```bash
python3 plugins/rcore-session-archive/scripts/setup_agents.py cursor --project /path/to/my-lab
```
...
`.opencode/plugins/rcore-session-archive.js`，归档脚本位于 `.opencode/rcore-hooks/`。
